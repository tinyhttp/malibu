...
describe('unsigned cookie - output', () => {
  it('should output a csrf token', async () => {
    const { fetch, server } = initApp({ middleware: 'cookie' })
    onTestFinished(() => {
      server.close()
    })
  ...
  it('should output a csrf token with given options (different salt & secret length)', async () => {
    ...
    const { fetch, server } = initApp({ middleware: 'cookie', options })
    onTestFinished(() => {
      server.close()
    })
...
describe('unsigned cookie - req.body', () => {
  it('should be able to pass through req.body', async () => {
    const { fetch, server } = initApp({ middleware: 'cookie', parser: 'json' })
    onTestFinished(() => {
      server.close()
    })
...
describe('unsigned cookie - req.query', () => {
...
describe('unsigned cookie - req.headers', () => {
